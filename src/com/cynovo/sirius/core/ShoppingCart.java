...
public class ShoppingCart implements DealModel.Callbacks {
	...
	public void addOrUpdateItems(OrderContentData mOrderContent) {
		...
		if(tmp != null && tmp.getProductId() != MANUID && tmp.getProductId() == mOrderContent.getProductId() && tmp.hasNoSpec())
		{
			tmp.addOneItem();
		}
		else
	...
	public void addNewItems(List<OrderContentData> mList) {
		...
		for(int i = 0; i < count ; i++)
		{
			...
			if(OrderContentList.size() > 0)
				tmp = OrderContentList.get(OrderContentList.size() - 1);
			if(tmp != null && tmp.getProductId() != MANUID && tmp.getProductId() == mList.get(i).getProductId() &&  tmp.hasNoSpec())
			{
...
//			if(!(mc.getProduct().getProductID() == MANUID && BasicArithmetic.compare(mc.getProduct().getPrice(), "0.00") == 0 && BasicArithmetic.compare(mc.getOnePrice(), "0.00") == 0))
			iterator.next();
			iterator.remove();
		}
		mOnsale.clear();
	...
	public boolean hasUnNewAddedManuItem()
	{
		...
		if(size > 0)
		{
			if(Double.parseDouble(OrderContentList.get(size-1).getItemAmount()) == 0 && OrderContentList.get(size-1).getProductId() == MANUID)
				return true;
		}
		return false;
	}
