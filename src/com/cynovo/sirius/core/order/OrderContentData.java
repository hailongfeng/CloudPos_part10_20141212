...
public class OrderContentData implements Cloneable {
	...
	private boolean isSubAttrDefault(ProductSubAttribute mProductSubAttribute) {
		if (mProductSubAttribute == null)
			return false;
		ProductSubAttribute tmp = ProductManager
				.getDefaultSubAttr(mProductSubAttribute.getProductAttributeID());
		if (tmp == null)
			return false;
		return tmp.getProductSubAttributeId() == mProductSubAttribute
		...
		if (mOnsale != null) {
			for (int i = 0; i < mOnsale.size(); i++) {
				...
				if (mode == 1) {
					...
					double rate = 1 + Double.parseDouble(BasicArithmetic.div(
							mOnsale.get(i).getValue(), "100"));
