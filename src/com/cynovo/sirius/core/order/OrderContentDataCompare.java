...
//		if((old.getmOrderContentRemark() == null && current.getmOrderContentRemark() == null) ||
//				(old.getmOrderContentRemark() == null && current.getmOrderContentRemark().getRemark().isEmpty()) ||
//				(old.getmOrderContentRemark().getRemark().isEmpty() && current.getmOrderContentRemark() == null) ||
//				((old.getmOrderContentRemark() != null && current.getmOrderContentRemark() != null) && 
...
//		}
	...
	static private int compareOnSale(List<Onsale> old,List<Onsale> current)
	{
		...
		for(int i = 0 ; i < old.size() ; i++)
		{
			for(int j = 0 ; j < current.size() ; j++)
			{
				if(old.get(i).getOnsaleID() == current.get(j).getOnsaleID())
				{
					power += ONSALE_SAME;
				}
			}
		}
		return power>ONSALE_MAX?ONSALE_MAX:power;
		*/
