...
//	static public int compare(OrderContentData old,OrderContentData current)
	...
	public static List<OrderChangeInfo>  compare(List<OrderContentData> old , List<OrderContentData> current)
	{
		...
		for(int i = 0 ; i < old.size() ; i++)
		{
			...
			{
				rlist.add(new OrderChangeInfo(old.get(i).getProductId(),old.get(i).getOnePrice(),i,j,true,false,0,OrderChangeInfo.NORMAL,"0.00" +
						"."));
			}
			else if(ret == OrderContentDataCompare.ONLY_COUNT_CHANGE)
			{
				int count = current.get(j).getCount() - old.get(i).getCount();
				rlist.add(new OrderChangeInfo(old.get(i).getProductId(),old.get(i).getOnePrice(),i,j,false,true,count,OrderChangeInfo.NORMAL,
						BasicArithmetic.sub(current.get(j).getItemAmount(), old.get(i).getItemAmount())));
			}
			else if(ret > OrderContentDataCompare.NOT_SAME && ret < OrderContentDataCompare.ONLY_COUNT_CHANGE)
			{
				int count = current.get(j).getCount() - old.get(i).getCount();
				rlist.add(new OrderChangeInfo(old.get(i).getProductId(),old.get(i).getOnePrice(),i,j,false,false,count,OrderChangeInfo.CHANGE,
						BasicArithmetic.sub(current.get(j).getItemAmount(), old.get(i).getItemAmount())));
			}
			else if(ret == OrderContentDataCompare.NOT_SAME)
			{
				int count = old.get(i).getCount();
				rlist.add(new OrderChangeInfo(old.get(i).getProductId(),old.get(i).getOnePrice(),i,-1,false,false,-count,OrderChangeInfo.DELETE,
						BasicArithmetic.sub("0.00", old.get(i).getItemAmount())));
			}
		...
		for(int j = 0 ; j < current.size() ; j++)
		{
			...
			rlist.add(new OrderChangeInfo(current.get(j).getProductId(),current.get(j).getOnePrice(),-1,j,false,false,count,OrderChangeInfo.ADD, 
					BasicArithmetic.sub(current.get(j).getItemAmount(),"0.00")));
...
//		
	...
	static  private IndexAndRet compareOneInList(OrderContentData oldone , List<OrderContentData> current,List<OrderChangeInfo> rlist)
	{
		int ret = 0;
		int index = -1;
		for(int j = 0 ; j < current.size() ; j++)
