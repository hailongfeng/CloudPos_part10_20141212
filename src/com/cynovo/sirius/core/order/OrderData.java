...
public class OrderData {
	...
	public void saveOrderContent() {
		...
		while (iterator.hasNext()) {
			...
			mc.setOrderId(currentOrder.getOrderID());
			if (mc.getProductId() == ShoppingCart.MANUID
					&& BasicArithmetic.compare(mc.getOnePrice(), "0") == 0)
				continue;
	...
	public synchronized int saveOrderData() {
		...
		if (orderId != -1) {
			...
			mOrderToAccount.setAccountName(AccountManager.getInstance()
					.getCurrentAccount().getNameToshow());
	...
	public void setPayBaseList(List<PayBase> mPayBaseList) {
		...
		while (iterator.hasNext()) {
			...
			mPayList.add(new OrdersToPaymentType(0, pb.getmType().toInt(), pb
					.getPayMoney()));
		}
	}
			...
			if (mOnsale != null) {
				for (int i = 0; i < mOnsale.size(); i++) {
					...
					if (mode == 1) {
						...
						double rate = 1 + Double.parseDouble(BasicArithmetic
								.div(mOnsale.get(i).getValue(), "100"));
