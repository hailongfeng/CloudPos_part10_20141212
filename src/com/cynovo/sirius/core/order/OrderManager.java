...
import com.cynovo.sirius.core.CardPayInfos;
...
public class OrderManager extends ManagerBase {
	...
	public static List<OrderContent> getOrderContentByOrderId1(int id) {
		...
		String sql = "SELECT oc.*, p.productCategoryID " +
				"FROM `OrderContent` oc, `Product` p " +
				"where orderID = " + id +
	...
	public static List<ProductSubAttribute> getProductSubAttributeByOrderContentId(int id) {
		...
		for (result.moveToFirst(); !result.isAfterLast(); result.moveToNext()) {
			...
			ProductSubAttribute psa = new ProductSubAttribute(productSubAttributeId,
					productAttributeID, name, priceAffect,enable);
			plist.add(psa);
		}
		result.close();
	...
	public static List<Onsale> getOnsaleByOrderContentId(int id) {
		String sql = "SELECT Onsale.*  FROM Onsale,OrderContentToOnsale  where OrderContentToOnsale.OrderContentID=" +String.valueOf(id)+
	...
	public static void addOrUpdateOrderRemark(OrderRemark mOrderRemark)
	{
		...
		else
		{
			...
			msqlitedb.updateWithOnConflict("OrderRemark", values, "orderID=?", whereArgs,  SQLiteDatabase.CONFLICT_REPLACE);
		}
		result.close();
	...
	public static Orders getOrderById(int id)
	{
		...
		Orders order = new Orders(orderID,billNo, createTime, modifyTime, OrderStatusID);
		result.close();
		return order;
	}
	...
	public static List<OrderContentData> getAllBusinessOrders(SQLiteDatabase sqlitedb)
	{
		String sql = "select pc.[productCategoryID], pc.[name],p.[productID],"
				+ "p.[name],p.[price],oc.[count],o.[orderID],oc.[orderContentID] ,"
				+ "o.[createTime] "+
				"from orders o,ordercontent oc,product p,productcategory pc "+
						"where o.[orderID]==oc.[orderID] and oc.[productID]=p.[productID] "+
						"and p.[productCategoryID]=pc.[productCategoryID] "+
						"order by pc.[productCategoryID] asc, o.[orderID] asc";
		
		Cursor result = sqlitedb.rawQuery(sql, null);
		List<OrderContentData> orderdatalist = new LinkedList<OrderContentData>();
		for (result.moveToFirst(); !result.isAfterLast(); result.moveToNext()) {
			int pcid = result.getInt(0);
			String pcname = result.getString(1);
			int pid = result.getInt(2);
			String pname = result.getString(3);
			double price=result.getDouble(4);
			int count=result.getInt(5);
			int oid = result.getInt(6);
			int ocid = result.getInt(7);
			long createTime=result.getLong(8);
			OrderContent oc=new OrderContent();
			oc.setProductId(pid);
			oc.setName(pname);
			oc.setAmount(count*price+"");
			oc.setCount(count);
			oc.setOrderId(oid);
			oc.setPrice(price);
			oc.setCreateTime(createTime);
			
			Product p=new Product(pid, pcid,pcname, pname, price+"", "", 1,1);
			
			OrderContentData orderdata=new OrderContentData(oc);
			orderdata.setProduct(p);
			List<ProductSubAttribute> psalist = getProductSubAttributeByOrderContentId(ocid);
			orderdata.setProductSubAttrList(psalist);
			
			List<Onsale> onsalelist = getOnsaleByOrderContentId(ocid);
			orderdata.setOnsale(onsalelist);
			
			OrderContentRemark remark = getOrderContentRemarkById(ocid);
			orderdata.setmOrderContentRemark(remark);
	...
	public static List<Orders> getAllOrders1() {
		...
		String sql = "SELECT * FROM `Orders` where OrderStatusID = " +  
				OrderStatus.ORDER_NORMAL.toInt() + " OR OrderStatusID = "+ 
				OrderStatus.ORDER_UNDO.toInt() + " ORDER BY modifyTime DESC";
	...
	public static String getDescriptionById(int id)
	{
		String sql = "SELECT name FROM `Product` WHERE productID IN " + 
				"(SELECT productID FROM `OrderContent` where orderID = " +
				String.valueOf(id) +")";
		Cursor result = msqlitedb.rawQuery(sql, null);
		String description = "";
	...
	public static List<Orders> getRecentOrders1() {
		...
		String sql = "SELECT * FROM `Orders` where OrderStatusID != " +  OrderStatus.ORDER_HANGUP.toInt() +
				" and OrderStatusID != "+ OrderStatus.ORDER_PERREFUND.toInt() + 
				" and OrderStatusID != "+ OrderStatus.ORDER_PRESTORE.toInt() + 
				" ORDER BY modifyTime DESC limit 30";
		Cursor result = msqlitedb.rawQuery(sql, null);
		List<Orders> odlist = new ArrayList<Orders>();
	...
	public static List<Orders> getOrdersByStatus1(int status) {
		String sql = "SELECT * FROM `Orders` where orderStatusID = "+String.valueOf(status) + " ORDER BY modifyTime DESC";
		Cursor result = msqlitedb.rawQuery(sql, null);
		List<Orders> odlist = new ArrayList<Orders>();
	...
	public static List<LiteOrderInfo> getAllOrdersInfoList()
	{
		...
		for(result.moveToFirst(); !result.isAfterLast(); result.moveToNext()) {
			...
			String remark = result.getString(8);
			String sti = result.getString(9);
			StringBuilder sb = new StringBuilder();
	...
	public static long saveOrderCustomerInfo(OrderCustomerInfo mMci)
	{
		...
		long insertId = msqlitedb.insertWithOnConflict("OrderCustomerInfo", null, values, SQLiteDatabase.CONFLICT_REPLACE);
		return insertId;
	}
	...
	public static long saveOrderProcess(OrderProcess mOrderProcess)
	{
	    ...
	    //int id = msqlitedb.updateWithOnConflict("OrderProcess",values,"orderID = -1 AND traceNo = "+mOrderProcess.getTraceNo(),null,SQLiteDatabase.CONFLICT_REPLACE);
	    String[] whereArgs =  {String.valueOf(mOrderProcess.getTraceNo())};
	    int id = msqlitedb.updateWithOnConflict("OrderProcess",values,"traceNo=?",whereArgs,SQLiteDatabase.CONFLICT_REPLACE);
	    if(id == 0)
	    {
	...
	public static long saveOrderContentToAttribute(OrderContentToAttribute orderContentToAttr)
	{
		...
		long insertId = msqlitedb.insertWithOnConflict("OrderContentToAttribute", null, values, SQLiteDatabase.CONFLICT_REPLACE);
		return insertId;
	}
	...
	public static long saveOrderContentToOnSale(OrderContentToOnsale onsale) 
	{
		...
		long insertId = msqlitedb.insertWithOnConflict("OrderContentToOnsale", null, values, SQLiteDatabase.CONFLICT_REPLACE);
		return insertId;
	}
	...
	public static void saveOrUpdateOrderContentRemark(OrderContentRemark mOrderContentRemark)
	{
	    ...
	    int id = msqlitedb.updateWithOnConflict("OrderContentRemark",values,"orderContentID=?",whereArgs,SQLiteDatabase.CONFLICT_REPLACE);
	    if(id == 0)
	    {
	...
	{
		String sql ="SELECT *FROM `OrderCustomerInfo` where orderProcessId= "
				+String.valueOf(orderProcessid);
	...
	public static OrderCustomerInfo getOrderCustomerInfoByProcessId(int orderProcessid)
	{
		...
		if (result.moveToFirst()){	
			...
			OrderCustomerInfo mOrderCustomerInfo = new OrderCustomerInfo(OrderProcessId,BankCardId,EmailInfo,Signature,TelephoneNo);	
		result.close();
...
//	}
	public static List<OrderContentToAttribute> getOrderContentToAttributesByOrderContentId(int OrderContentId)
	{
		String sql ="SELECT octa.*, psa.productAttributeID " +
				"FROM `OrderContentToAttribute` octa, ProductSubAttribute psa " +
				"WHERE octa.orderContentID = " + String.valueOf(OrderContentId) +
				" AND octa.productSubAttributeID = psa.productSubAttributeID ";
	...
	{
		String sql ="SELECT * FROM `OrderContentToOnsale` where OrderContentId="
				+String.valueOf(OrderContentId);
		Cursor reCursor =msqlitedb.rawQuery(sql, null);
	...
	{
		String sql = "SELECT * FROM `OrderToAccount` where orderID = "
