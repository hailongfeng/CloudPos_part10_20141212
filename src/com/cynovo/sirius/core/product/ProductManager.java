package com.cynovo.sirius.core.product;
...
import com.cynovo.sirius.core.ManagerBase;
...
public class ProductManager extends ManagerBase {
	...
	public static int saveProductData(ProductData mProductData) {
		...
		try {
		    if(productHasSame(mProductData.getProduct())) {
		    	id = (int)msqlitedb.insertWithOnConflict("Product", null, values,
		    ...
		    for(int i = 0 ; i < mProductData.getAttrList().size() ; i++) {
		    	...
		    	msqlitedb.insertWithOnConflict("ProductToAttribute", null, valuesAttr,
		    			SQLiteDatabase.CONFLICT_REPLACE);
		    }
	...
	public static void deleteProductCate(int productCate) {
		...
		try {
			...
			String sql0 = "UPDATE `ProductCategory` SET enable=0,syncFlag=1 where `productCategoryID` = "
					+ productCate;
			msqlitedb.execSQL(sql0);
			msqlitedb.setTransactionSuccessful();
		} catch (Exception e) {
	...
	public static int addProductCate(ProductCategory mProductCategory) {
	    ...
	    values.put("syncFlag",0);
	    return (int)msqlitedb.insertWithOnConflict("ProductCategory", null, values,
	    		SQLiteDatabase.CONFLICT_REPLACE);
	}
	...
	public static int updateProductCate(ProductCategory mProductCategory) {
		...
		try {
		    ...
		    String sql0 = "UPDATE `Product` SET productCategoryID=" + id +
		    		" , syncFlag=1 where `productCategoryID` = "+mProductCategory.getProductCategoryId();
			msqlitedb.execSQL(sql0);	
	...
	public static boolean productCateAlreadyIn(ProductCategory mProductCategory) {
		String sql = "SELECT COUNT(*) FROM `ProductCategory` where productCategoryID != " +
				mProductCategory.getProductCategoryId() + " and name = '" +
				mProductCategory.getProductCategoryName()+"'"+ " and enable=1";
	...
	public static int addProduct(Product mProduct) {
	    ...
	    values.put("enable",1);
	    values.put("syncFlag",0);
	    return (int)msqlitedb.insertWithOnConflict("Product", null, values,
	    		SQLiteDatabase.CONFLICT_REPLACE);
	}
	...
	public static int updateProduct(Product mProduct) {
		...
		String sql1 = "UPDATE `Product` SET enable = 0,syncFlag=1 where `productID` = " +
				mProduct.getProductID();
			msqlitedb.execSQL(sql0);
			msqlitedb.execSQL(sql1);
			msqlitedb.setTransactionSuccessful();
	...
	public static int addProductAttr(ProductAttribute mProductAttr) {
	    ...
	    return (int)msqlitedb.insertWithOnConflict("ProductAttribute", null, values,
	    		SQLiteDatabase.CONFLICT_REPLACE);
	}
	...
	public static int updateProductAttr(ProductAttribute mProductAttr) {
		...
		try {
		    ...
		    id = (int)msqlitedb.insertWithOnConflict("ProductAttribute", null, values,
		    		SQLiteDatabase.CONFLICT_REPLACE);
		    
		    String sql0 = "UPDATE `ProductToAttribute` SET productAttributeID="+id +
		    		" where `productAttributeID` = "+mProductAttr.getProductAttributeID();
			msqlitedb.execSQL(sql0);	
	...
	public static void updateProductDefaultSub(ProductAttribute mProductAttr,int defaultid) {
		String sql0 = "UPDATE `ProductAttribute` SET defaultChoice=" + defaultid +
				" where `productAttributeID` = "+mProductAttr.getProductAttributeID();
		msqlitedb.execSQL(sql0);
	}
	...
	public static int updateOnsale(Onsale mOnsale) {
		...
		try {
		    id = (int)msqlitedb.insertWithOnConflict("Onsale", null, values,
		    		SQLiteDatabase.CONFLICT_REPLACE);
		    
		    String sql1 = "UPDATE `Onsale` SET enable = 0,syncFlag=1 where `onsaleID` = " +
		    		mOnsale.getOnsaleID();
			msqlitedb.execSQL(sql1);
			msqlitedb.setTransactionSuccessful();
		} catch (Exception e) {
	...
	public static ProductSubAttribute getDefaultSubAttr(int attrId) {
		String sql = "SELECT * FROM `ProductSubAttribute` where `enable`=1 and `productSubAttributeId` in " +
				"(SELECT defaultChoice FROM `ProductAttribute` where `enable`=1 and productAttributeID = "+
				attrId + ")" ;
		Cursor result = msqlitedb.rawQuery(sql, null);
		if(!result.moveToFirst()) {
			result.close();
		...
		result.close();
		ProductSubAttribute p = new ProductSubAttribute(productSubAttributeId, productAttributeID,
				name, priceAffect,enable);
		return p;
	}
	...
	public static ProductAttribute getProductAttributeByAttrId(int attrId) {
		String sql = "SELECT * FROM `ProductAttribute` where `enable`=1 and `productAttributeID` = "
				+ attrId;
		Cursor result = msqlitedb.rawQuery(sql, null);
		if (!result.moveToFirst()) {
			result.close();
	...
	public static List<ProductAttribute> getProductAttributeById(int productId ,boolean onlyenable)
	{
		...
		for (result.moveToFirst(); !result.isAfterLast(); result.moveToNext()) {
			productlist.add(new ProductAttribute(result.getInt(0), result
					.getString(1), result.getInt(2), result.getInt(3), result.getInt(4)));
		}
		result.close();
	...
	public static List<ProductAttribute> getProductAttributeForSync() {
		...
		for (result.moveToFirst(); !result.isAfterLast(); result.moveToNext()) {
			...
			ProductAttribute p = new ProductAttribute(productAttributeID, name,
					choiceType, defaultChoice, enable);
	...
	public static List<ProductSubAttribute> getProductSubAttributeByAttrId(int attrId) {
		String sql = "SELECT * FROM `ProductSubAttribute` where `enable`=1 and `productAttributeID` = "
				+ attrId;
		Cursor result = msqlitedb.rawQuery(sql, null);
		List<ProductSubAttribute> productlist = new LinkedList<ProductSubAttribute>();

		for (result.moveToFirst(); !result.isAfterLast(); result.moveToNext()) {
			productlist.add(new ProductSubAttribute(result.getInt(0), result
					.getInt(1), result.getString(2), result.getString(3), result.getInt(4)));
		}
		result.close();
	...
	public static List<ProductSubAttribute> getProductsSubAttributesForSync() {
		...
		for (result.moveToFirst(); !result.isAfterLast(); result.moveToNext()) {
			...
			int enable = result.getInt(4);
			int syncFlag = result.getInt(5);
			ProductSubAttribute p = new ProductSubAttribute(
					productSubAttributeId, productAttributeID, name, priceAffect, enable);
			p.setSyncFlag(syncFlag);
	...
	public static List<Product> getALLProductByCategory(int categoryId) {
		...
		String sql = "SELECT * FROM `Product` where `enable`=1 and productCategoryID = "
				+ String.valueOf(categoryId) + " and productID > 0 ORDER BY name ASC ";
		Cursor result = msqlitedb.rawQuery(sql, null);
	...
	public static int getProductCountByCategory(int categoryId) {
		...
		if (categoryId != 0)
			...
			tmpByCate = "";
		String sql = "SELECT COUNT(*) FROM `Product` where `enable`=1 and "
				+ tmpByCate + " productID > 0 ORDER BY name ASC ";
		Cursor result = msqlitedb.rawQuery(sql, null);
		if (!result.moveToFirst()) {
			result.close();
			return 0;
	...
	public static int getProductCountByAttr(int attrId) {
		String sql = "SELECT COUNT(*) FROM `ProductToAttribute` where  `enable`=1 and productAttributeID = "
				+ attrId;
		Cursor result = msqlitedb.rawQuery(sql, null);
		if (!result.moveToFirst()) {
			result.close();
	...
	public static List<Product> getALLProduct() {
		...
		for (result.moveToFirst(); !result.isAfterLast(); result.moveToNext()) {
			...
			int enable = result.getInt(5);
			int syncFlag = result.getInt(6);
			Product p = new Product(productID, productCategoryID, name, price,
					productPhoto, enable);
			p.setSyncFlag(syncFlag);
			productlist.add(p);
		}
	...
	public static Onsale getOnSaleByid(int id) {
		...
		Onsale mOnsale = new Onsale(result.getInt(0), result.getInt(1),
	...
	public static List<ProductPinyin> getProductPinyinList() {
		...
		for (result.moveToFirst(); !result.isAfterLast(); result.moveToNext()) {
			...
			ProductPinyin p = new ProductPinyin(productID, productCateID,
					productname, firstchar, null, null);
			pinyinList.add(p);
		}
		result.close();
	...
	public static List<ProductPinyin> getProductPinyinListByCataID(int id) {
		String sql = "select * from ProductPinyin where enable = 1 and productCategoryID = "
				+ id + " order by upper(firstChar) asc";
		Cursor result = msqlitedb.rawQuery(sql, null);
		List<ProductPinyin> pinyinList = new ArrayList<ProductPinyin>();
	...
	public static void InitPinyinTable() {
		...
		try {
			...
			while (iterator.hasNext()) {
				...
				msqlitedb.insertWithOnConflict("ProductPinyin", null, values,
	...
	public static boolean deleteAllProduct() {
		final String delete_OrderContent = "delete from ordercontent";
		final String delete_OrderContentRemark = "delete from OrderContentRemark";
		final String delete_OrderContentToAttribute = "delete from OrderContentToAttribute";
		final String delete_OrderContentToOnsale = "delete from OrderContentToOnsale";
		final String delete_Orders = "delete from Orders";
		final String delete_Onsale = "delete from Onsale";
		final String delete_Product = "delete from Product";
		final String delete_ProductCategory = "delete from ProductCategory";
		final String delete_ProductSubAttribute = "delete from ProductSubAttribute";
		final String delete_ProductAttribute = "delete from ProductAttribute";
		final String delete_ProductProductPinyin = "delete from ProductPinyin";
		final String delete_OrderToOnsale = "delete from OrderToOnsale";
		final String delete_ProductToAttribute = "delete from ProductToAttribute";
		final String delete_OrderToAccount = "delete from OrderToAccount";
		final String delete_OrdersToPaymentType = "delete from OrdersToPaymentType";
		final String delete_OrderProcess= "delete from OrderProcess";
		final String delete_OrderCustomerInfo= "delete from OrderCustomerInfo";
		final String delete_OrderRemark= "delete from OrderRemark";
		msqlitedb.beginTransaction();
		boolean ret = false;
		try {
			msqlitedb.execSQL(delete_OrderContent);
			msqlitedb.execSQL(delete_OrderContentRemark);
			msqlitedb.execSQL(delete_OrderContentToAttribute);
			msqlitedb.execSQL(delete_OrderContentToOnsale);
			msqlitedb.execSQL(delete_Orders);
			msqlitedb.execSQL(delete_Onsale);
			msqlitedb.execSQL(delete_Product);
			msqlitedb.execSQL(delete_ProductCategory);
			msqlitedb.execSQL(delete_ProductSubAttribute);
			msqlitedb.execSQL(delete_ProductAttribute);
			msqlitedb.execSQL(delete_ProductProductPinyin);
			msqlitedb.execSQL(delete_OrderToOnsale);
			msqlitedb.execSQL(delete_ProductToAttribute);
			msqlitedb.execSQL(delete_OrderToAccount);
			msqlitedb.execSQL(delete_OrdersToPaymentType);
			msqlitedb.execSQL(delete_OrderProcess);
			msqlitedb.execSQL(delete_OrderCustomerInfo);
			msqlitedb.execSQL(delete_OrderRemark);
			msqlitedb.setTransactionSuccessful();
			ret = true;
		} catch (Exception e) {
			e.printStackTrace();
