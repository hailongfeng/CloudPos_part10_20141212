...
public class SettingsManager extends ManagerBase {
	...
	public static Settings querySettings() {
		...
		if(retval) {
			settings = new Settings(result.getString(0),result.getInt(1),result.getString(2),result.getInt(3),result.getString(4),result.getString(5));
		}
		result.close();
		return settings;
	}
...
//
//		Cursor result = msqlitedb.rawQuery("SELECT merchantCode FROM `Settings`", null);
//		String merchantNo = "";
//		boolean retval = result.moveToFirst();
//		if(retval == false)
//			return null;
...
//
//		Cursor result = msqlitedb.rawQuery("SELECT terminalCode FROM `Settings`", null);
//		String terminalNo = "";
//		boolean retval = result.moveToFirst();
//		if(retval == false)
//			return null;
