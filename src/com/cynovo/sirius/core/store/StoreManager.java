package com.cynovo.sirius.core.store;
...
public class StoreManager  extends ManagerBase  {
	...
	public static Store getStore()
	{
		...
		String storeName = (result.getString(1) == null || result.getString(1).length() == 0 ||
				result.getString(1).equals("null"))?"":result.getString(1);
		int storeAddressID = result.getInt(2);
		String storePhoto = result.getString(3);
		int companyAddressID = result.getInt(4);
		String companyName = (result.getString(5) == null || result.getString(5).length() == 0 ||
				result.getString(5).equals("null"))?"":result.getString(5);
		String companyPhoto = result.getString(6);
		String nickName = (result.getString(7) == null || result.getString(7).length() == 0 ||
				result.getString(7).equals("null"))?"":result.getString(7);
		String address = (result.getString(8) == null || result.getString(8).length() == 0 ||
				result.getString(8).equals("null"))?"":result.getString(8);
		String caddress = (result.getString(9) == null || result.getString(9).length() == 0 ||
				result.getString(9).equals("null"))?"":result.getString(9);
	...
	public static Store getStorePic(){
		...
		mStore = new Store(storeID,storeAddressID,companyAddressID,storeName,storePhoto,companyName,
				companyPhoto,nickName,address,caddress);
		result.close();
		return mStore;
	}
	...
	public static void updateStoreInfo(String nickName, String companyName,
			String storeAddress, String companyAddress) {
		String sql = "update Store set nickName='"+nickName+"',companyName='"+companyName+
				"',companyAddress='"+companyAddress+"',address='"+storeAddress+"'";
		msqlitedb.execSQL(sql);
	}
