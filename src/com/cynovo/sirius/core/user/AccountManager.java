package com.cynovo.sirius.core.user;
...
import com.cynovo.sirius.core.BaseInfoManager;
	...
	public int saveAccount(Account mAccount) {
		...
		if(checkHasSame(mAccount)) {//修改
			String sql = "UPDATE `Account` SET accountPrivilegeID="+mAccount.getAccountPrivilege()+
					...
					"',account='"+mAccount.getAccount()+"',pass='"+mAccount.getPass()+"'"+
		    ...
		    if(mAccount.getAccountID() < 0 && length > 6) {
				...
				try {
					sha1pass = Common.SHA1.toSHA1(mAccount.getAccount().substring(length-6, length)).
							toUpperCase();
