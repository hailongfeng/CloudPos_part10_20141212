...
public class AddEmployeeFragment extends AbstractLoadingFragment implements OnEditorActionListener,
		OnClickListener {
	...
	private String[] PRAVALG1 = {Common.getString(R.string.shop_manager),
			Common.getString(R.string.shop_assistant)};
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container,
		...
		ArrayAdapter<String> spCertTypeAdapter = new ArrayAdapter<String>(
				getActivity(), android.R.layout.simple_spinner_dropdown_item, spinArray);
	...
	private void saveAccount() {
		if (firstNameEdit.getText() == null ||
			...
			Toast.makeText(CloudPosApp.getInstance(), R.string.alert_adduser_namenotnull,
					Toast.LENGTH_LONG).show();
			return;
		} else if(lastNameEdit.getText() == null ||
			...
			Toast.makeText(CloudPosApp.getInstance(), R.string.password_hint,
					Toast.LENGTH_LONG).show();
			return;
		} else if(!pswEdit.getText().toString().trim().equals(checkPswEdit.getText().toString().trim())) {
			Toast.makeText(CloudPosApp.getInstance(), R.string.psw_different,
					Toast.LENGTH_LONG).show();
			return;
		} else if(pswEdit.getText().toString().length() < 6 ||
				pswEdit.getText().toString().length() > 16) {
			Toast.makeText(CloudPosApp.getInstance(), R.string.psw_lenth_hint,
					Toast.LENGTH_LONG).show();
			return;
		}
...
//		CynovoHttpClient.post("api/account/add_with_date.php", params, new JsonHttpResponseHandler() {
			...
			public void onSuccess(int statusCode, JSONObject response) {
				...
				try {
					...
					if (ret == 0) {
						...
						if (ret1 == -1) {
							Toast.makeText(CloudPosApp.getInstance(), R.string.alert_adduser_fail,
