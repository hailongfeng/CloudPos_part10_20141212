...
public class EditEmployeeFragment extends AbstractLoadingFragment implements OnEditorActionListener,
	...
	private String[] PRAVALG1 = {Common.getString(R.string.shop_assistant),
			Common.getString(R.string.shop_manager)};
	...
	private void saveAccount() {
		if (firstNameEdit.getText() == null || firstNameEdit.getText().toString().trim().isEmpty()) {
			Toast.makeText(CloudPosApp.getInstance(), R.string.first_name_hint,
					Toast.LENGTH_SHORT).show();
			return;
		}
		if (lastNameEdit.getText() == null || lastNameEdit.getText().toString().trim().isEmpty()) {
			Toast.makeText(CloudPosApp.getInstance(), R.string.last_name_hint,
					Toast.LENGTH_SHORT).show();
			return;
		}
	...
	private void deleteAccount() {
		if (editAccount == null || !(accountManager.getCurrentAccount().getAccountPrivilege() <
				editAccount.getAccountPrivilege())) {
			return;
		}
			...
			public void onSuccess(int statusCode, JSONObject response) {
				...
				try {
					...
					if (ret == 0) {
						...
						Toast.makeText(CloudPosApp.getInstance(), R.string.delete_employee_success,
								Toast.LENGTH_SHORT).show();
	...
	public boolean onTouch(View v, MotionEvent event) {
		...
		case R.id.usrmg_scrollview:
			...
			if (event.getAction() == MotionEvent.ACTION_DOWN) {
				InputMethodManager inputManager = (InputMethodManager) mUsrmg_Scrollview
