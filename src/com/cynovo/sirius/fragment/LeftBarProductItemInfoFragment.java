...
import com.cynovo.sirius.RefundActivity;
import com.cynovo.sirius.SaleActivity;
import com.cynovo.sirius.adapter.OrderDetailAdapter;
import com.cynovo.sirius.core.DealModel;
import com.cynovo.sirius.core.order.AnalizeOrderProcess;
...
//		mOrderData = OrderManager.getOrderDataById(orderid);
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container,
		...
		leftBarProductItemLayout = inflater.inflate(
				R.layout.leftbarproductiteminfo1, container, false);
		leftbar_checkout_deal_btn = (Button) leftBarProductItemLayout
				.findViewById(R.id.leftbar_checkout_deal);
		...
		mOrderDetailAdapter = new OrderDetailAdapter(getActivity(),
				mOrderData.getOrderContentList());
	...
	private void InitView() {
		...
		if ((mOrderData.getmOrderStatus() == OrderStatus.ORDER_PRESTORE)
			...
			if (process != null) {
				...
				if (AnalizeOrderProcess.getOrderAleradyPayed(orderid,
							process.getOrderProcessModeId(), mOrderData.getAmount())) {
					AnalizeOrderProcess.solveOrderAleradyPayed(orderid,
							process.getOrderProcessModeId(), mOrderData.getAmount());
				}
				mPayMethod.setText(R.string.info_input_card);
				leftbar_checkout_deal_btn.setVisibility(View.GONE);
		...
		mDateView.setText(Common.getDateTimeString(mOrderData.getCurrentOrder()
				.getCreateTime()));
		String acN = "";
	...
	public void onClick(View v) {
		...
		case R.id.left_button:
			...
			if (mOrderData.getPayType() != PayBase.PAYTYPE.PAY_BY_CASH.toInt()
				...
				mDeleteHangupDialog = new AnyWhereDialog(getActivity(), 540,
						280, 0, 0, R.layout.confirm_delete,
				...
				mDeleteHangupDialog.show();
					...
					|| (mOrderData.getPayType() == PayBase.PAYTYPE.PAY_BY_MSR.toInt()
					&& restoreType == PRESTORETYPE2)) { // 冲正//TODO:需要其他的判断信息
				// ReverseActivity
		...
		case R.id.leftbar_checkout_deal:
			if (mOrderData.getCardPayInfos() != null &&
					mOrderData.getCardPayInfos().getOrderProcess() != null)
				mLeftBarFragment.addLeftBarProductBilling(
						mOrderData.getCardPayInfos().getOrderProcess().getOrderProcessId());
			break;
		case R.id.right_button:
			if (level == 1 && mOrderData.getmOrderStatus() == OrderStatus.ORDER_HANGUP) {
				if (mOrderData != null) {
					if (DealModel.getInstance().getOrderData()
							.getInitOlderHangUpList() != null) {
						mContCoverDialog = new AnyWhereDialog(getActivity(),
								540, 280, 0, 0, R.layout.alert_xml,
								R.style.Theme_dialog1, Gravity.LEFT | Gravity.TOP, true);
					...
					|| mOrderData.getmOrderStatus() == OrderStatus.ORDER_PERREFUND) { // TODO:need check
				...
				Bundle translateBundle = ActivityOptions.makeCustomAnimation(
						LeftBarProductItemInfoFragment.this.getActivity(),
						android.R.anim.fade_in, android.R.anim.fade_out).toBundle();
				refundintent.putExtra("orderid", orderid);
				LeftBarProductItemInfoFragment.this.getActivity()
						.startActivity(refundintent, translateBundle);
			} else
				return;
			break;
		case R.id.i_know:
	...
	private void setListViewHeightBasedOnChildren(ListView listView) {
		...
		if (listAdapter.getCount() > 0)
			totalHeight = listAdapter.getView(0, null, listView).getMinimumHeight() *
