...
import com.cynovo.sirius.widget.LoginHelpBar;
...
public class LoginPageFragment extends AbstractSupportV4Fragment implements TextView.OnEditorActionListener,
	...
	int[] anmiArray = { R.anim.push_up_in, R.anim.push_up_out,
...
////		Common.FlipperShowIndex(getActivity(), userViewFlipper, index, anmiArray);
...
//		userViewFlipper.setDisplayedChild(index);
	...
	public void refresh(Context context) {
		...
		try {
			if(StoreManager.getStore().getNickName() != null &&
					StoreManager.getStore().getNickName().length() >0 &&
...
////			if(mUserName.getText() != null && mUserName.getText().toString().trim().length() == 11) {
////				intent.putExtra("phone", mUserName.getText().toString().trim());
////			}
		...
		if (2 == result) {
			Intent saleintent = new Intent(LoginPageFragment.this.getActivity(),SaleActivity.class);
       		Bundle translateBundle = ActivityOptions.makeCustomAnimation(getActivity(),
       				R.anim.push_left_in,R.anim.push_left_out).toBundle();
