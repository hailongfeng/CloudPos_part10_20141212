...
public class MemberAddOneFragment extends AbstractDialogFragment implements OnClickListener,
	...
	private void checkPhoneNo() {
		if(memberAddNameEdit.getText() == null ||
				memberAddNameEdit.getText().toString().trim().length() == 0 ||
				memberAddPhoneEdit.getText() == null ||
				memberAddPhoneEdit.getText().toString().trim().length() == 0) {
		...
		CynovoHttpClient.post("api/membership/check_phone_no.php", params,
