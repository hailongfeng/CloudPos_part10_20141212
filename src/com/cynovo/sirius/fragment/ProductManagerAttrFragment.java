...
import com.cynovo.sirius.adapter.PriceAdapte;
 ...
 * @author ycb
 */
public class ProductManagerAttrFragment extends AbstractFragment implements
		OnClickListener, OnItemClickListener, OnCheckedChangeListener, OnEditorActionListener {
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container,
		...
		result = inflater.inflate(R.layout.productmanagerattr_layout,
				container, false);
		mListView = (ListView) result.findViewById(R.id.prdmg_attr_listView);
		mPrdmg_attr_add_btn = (Button) result
				.findViewById(R.id.prdmg_attr_add_btn);
		mAttrDeleteBtn = (Button) result
				.findViewById(R.id.prdmg_attr_delete_btn);
		mAttrName = (EditText) result
				.findViewById(R.id.prdmg_attr_add_inputedit);
		...
		mBelowLine = (ImageView) result
				.findViewById(R.id.prdmg_attr_imageview_below);

		mSaveBtn = (Button) result.findViewById(R.id.pdtmg_add_attr_button);
	...
	public void onViewCreated(View view, Bundle savedInstanceState) {
		...
		mProductSubAttributeList = ProductManager.getProductSubAttributeByAttrId(
				mProductAttribute.getProductAttributeID());
		...
		if (attrNameStr != null && !attrNameStr.isEmpty()) {
			...
			if (id == -1) {
				Toast.makeText(CloudPosApp.getInstance(),
						R.string.alert_addattr_fail, Toast.LENGTH_SHORT).show();
			} else {
				Toast.makeText(CloudPosApp.getInstance(),
						R.string.alert_addattr_success, Toast.LENGTH_SHORT).show();
				...
				if (mProductSubAttributeList != null) {
					...
					for (int i = 0; i < mProductSubAttributeList.size(); i++) {
						...
						int subid = ProductManager
								.saveProductSubAttr(mProductSubAttributeList.get(i));
			...
			Toast.makeText(CloudPosApp.getInstance(),
					R.string.alert_addattr_notnull, Toast.LENGTH_SHORT).show();
		}
	}

	public void remove() {
		if (mProductAttribute != null
				&& mProductAttribute.getProductAttributeID() > 0) {
	...
	public void addNewSubAttr(ProductSubAttribute mProductSubAttribute) {
		if (mProductSubAttributeList.size() > 8) {
			Toast.makeText(CloudPosApp.getInstance(),
					R.string.alert_addattr_toomany, Toast.LENGTH_SHORT).show();
			return;
		}
		if (!checkSameInSubList(mProductSubAttribute)) {
			...
			Toast.makeText(CloudPosApp.getInstance(), R.string.alert_addattr_alreadyin,
	...
	public void onClick(View v) {
		...
		case R.id.prdmg_attr_add_btn:
			// init();
			mDialog = new AnyWhereDialog(getActivity(), 540, 280, -100, -70,
					R.layout.attribute_factor_layout, R.style.Theme_dialog1,
		...
		case R.id.cancel_delete:
			mDelDialog.dismiss();
			break;
		case R.id.pdtmg_add_attr_button:
			save();
			break;
		}
	}
