...
import com.cynovo.sirius.network.NetworkUtil;
...
import com.cynovo.sirius.util.update.UpdateApk;

public class SettingUpdateFragment extends AbstractLoadingFragment implements OnClickListener {
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container,
		...
		result = inflater.inflate(R.layout.update_checkupdate, container, false);
		...
		if(MySharedPreferencesEdit.getInstancePublic(getActivity()).getLastUpdateVersion() <
				GetVersionCode.getAppVersionCode(getActivity())) {
			if(MySharedPreferencesEdit.getInstancePublic(getActivity()).getUpdateApkTime() > 0) {
				SimpleDateFormat sdf = new SimpleDateFormat(Common.DATE_TIME_FORMAT_WITH_SECONDS,
						Locale.getDefault());
				Date date = new Date(MySharedPreferencesEdit.getInstancePublic(getActivity()).
						getUpdateApkTime());
