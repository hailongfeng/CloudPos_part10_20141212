...
import com.cynovo.sirius.util.FileManager;
import com.cynovo.sirius.widget.MyImageButton;

/**
 * 商品列表界面
 */
public class ShowAllProductFragment extends AbstractFragment implements OnClickListener {
	...
	private void initAndset() {
		...
		mProductList = (ProductListFragment) fragmentManager.findFragmentById(R.id.ProductListFragment);
	}

	@SuppressWarnings("deprecation")
	private void inti_status_button() {
		mProductListButton.setText(0);
		mProductListButton.setBackgroundDrawable(FileManager
				.readBitMapDrawable(getActivity(), R.drawable.goods_list_button1));
		mManual_input.setBackgroundDrawable(FileManager.readBitMapDrawable(
				getActivity(), R.drawable.keyboard_button1));
		...
		mProductListButton.setOnTouchListener(new OnTouchListener() {
			...
			public boolean onTouch(View v, MotionEvent event) {
				...
				if (event.getAction() == MotionEvent.ACTION_DOWN) {
					mManual_input.setBackgroundDrawable(FileManager
							.readBitMapDrawable(getActivity(),
