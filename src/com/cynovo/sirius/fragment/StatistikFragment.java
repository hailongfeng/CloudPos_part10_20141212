...
import com.cynovo.sirius.adapter.MyViewPagerAdapter;
import com.cynovo.sirius.adapter.ProductStatisticAdapter;
import com.cynovo.sirius.core.order.StatisticsItem;
import com.cynovo.sirius.core.order.StatisticsManager;
import com.cynovo.sirius.core.product.SaleProductInfo;
import com.cynovo.sirius.core.store.StoreManager;
import com.cynovo.sirius.sqlite.MySharedPreferencesEdit;
import com.cynovo.sirius.txt.ItemsTxt;
import com.cynovo.sirius.txt.SalesTxt;
import com.cynovo.sirius.ui.widget.date.time.ScreenInfo;
import com.cynovo.sirius.ui.widget.date.time.WheelMain;
import com.cynovo.sirius.util.CalendarUtil;
...
import com.cynovo.sirius.widget.StatistikListView;
...
public class StatistikFragment extends LeftBarBaseFragment /*implements OnCheckedChangeListener*/ {
	...
	public void onResume() {
		List<SaleProductInfo> mplistInfo = StatisticsManager.getSaleProductInfo(
				startButton.getText().toString().trim(),
				endButton.getText().toString().trim());
	...
	private void InitImageView(View v) {
		...
		int screenW = 484;//dm.widthPixels;// 获取分辨率宽度
	...
	private void showSelectDateTimeDialog(final boolean isStartTime) {
		...
		if(!isStartTime) { // end time
			Date date = null;
			try {
				date = sdf.parse(endButton.getText().toString().trim());
			...
			Date date = null;
			try {
				date = sdf.parse(startButton.getText().toString().trim());
				calendar.setTimeInMillis(date.getTime());
			} catch (Exception e) {
				e.printStackTrace();
			}
		...
		sureButton.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View arg0) {
				...
				if(isStartTime) { // start time
					startButton.setText(selectTime);
					mStatistikListView.setStatistics(StatisticsManager.getStatisticsItem(
							startButton.getText().toString().trim(), endButton.getText().toString().trim()));
...
//					}
	...
	private int calculateDays() {
	    try {
			...
			long to = df.parse(endButton.getText().toString()).getTime();
			long from = df.parse(startButton.getText().toString()).getTime();
