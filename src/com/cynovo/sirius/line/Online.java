...
//			List<OrderProcess> orderProcesses = OrderManager.getOrderProcessById1(orderId);
//			List<OrderCustomerInfo> orderCustomerInfos = new ArrayList<OrderCustomerInfo>();
...
//				OrderCustomerInfo oci = OrderManager.getOrderCustomerInfoByProcessId(orderProcessesId);
...
//			List<OrdersToPaymentType> ordersToPaymentTypes = OrderManager.getOrdersPaymentTypeById(orderId);
//			List<OrderContent> orderContents = OrderManager.getOrderContentByOrderId1(orderId);
//			List<OrderContentToAttribute> orderContentToAttribute = new ArrayList<OrderContentToAttribute>();
//			List<OrderContentToOnsale> orderContentToOnsale = new ArrayList<OrderContentToOnsale>();
...
//			Type ordersToPaymentType = new TypeToken<List<OrdersToPaymentType>>(){}.getType();
//			String otptString = gson.toJson(ordersToPaymentTypes, ordersToPaymentType);
//			//Type orderProcessType = new TypeToken<List<OrderProcess>>(){}.getType();
//			String opString = gson.toJson(orderProcesses);
//			Type orderCustomerInfoType = new TypeToken<List<OrderCustomerInfo>>(){}.getType();
//			String ociString = gson.toJson(orderCustomerInfos, orderCustomerInfoType);
//			Type orderContentType = new TypeToken<List<OrderContent>>(){}.getType();
//			String ocString = gson.toJson(orderContents, orderContentType);
//			Type octaType = new TypeToken<List<OrderContentToAttribute>>(){}.getType();
...
//
//			String orderstring = cpuid + "&order="+ orderString +"&otpt=" +otptString +"&op=" +opString 
//					+"&oci=" +ociString+"&oc=" + ocString +"&octa=" +octaString 
//					+"&octo=" +octoString +"&bc=" +bcString;
//			String ret = NetworkCommunication.postRequest("order/create.php", orderstring);
//			
//			if(ret != null) {
...
//			List<OrderProcess> orderProcesses = OrderManager.getOrderProcessById1(order.getOrderID());
//			
...
//		
//		Gson gson = new GsonBuilder().excludeFieldsWithoutExposeAnnotation().create();
//		List<ProductCategory> pc = ProductManager.getProductCategoryForSync();
...
//		
//		List<ProductAttribute> pa = ProductManager.getProductAttributeForSync();
...
//
//		
//		List<ProductSubAttribute> psa =ProductManager.getProductsSubAttributesForSync();
...
//		}
	...
	public int loginProceess(Account mAccount) {
		/*int loginResult = 0;
		{
			...
			{
				String ret = NetworkCommunication.postRequest("control/login.php",
						cpuid + "&acc=" + mAccount.getAccount() + "&pass=" + mAccount.getPass());

				if (ret != null) {
					Gson gson = new Gson();
					Result result = gson.fromJson(ret, Result.class);
					HttpSession session = gson.fromJson(ret, HttpSession.class);

					if (result.getRet() == 0) {
						if (session.getSid() != null) {
							Log.i("session_id", session.getSid());
							sid = session.getSid();
							mAccount.setAccountPrivilege(session.getPrivilege());
						//	AccountManager.getInstance().getmBaseInfo().setAllBaseInfo(session.getMerchantNo(), session.getTerminalNo(), session.getSid());
						}
						loginResult = 2;
					}
				} 
