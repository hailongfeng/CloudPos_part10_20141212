...
import com.cynovo.sirius.LoginInActivity;
import com.cynovo.sirius.R;
import com.cynovo.sirius.core.user.AccountManager;
import com.cynovo.sirius.widget.UserDialog;
...
//		   munlockUI = (RelativeLayout) titalButton.findViewById(R.id.useraccountshadebackground);
		 
	}

	@SuppressLint("NewApi")
	public void show(){
		dialog = new UserDialog(mContex, 321, 245, -540, -200, R.layout.useraccount, R.style.Theme_dialog1, Gravity.LEFT, true);
		...
		//edit = (EditText) root.findViewById(R.id.unlock_user_account_edittext);
	...
	private void input_unlock_user_account_botton(){
		...
		InputMethodManager inputManager =   
				(InputMethodManager)unlock_saleactivity_edittext.getContext().getSystemService(Context.INPUT_METHOD_SERVICE);   
		 ...
		 unlock_saleactivity_edittext.setOnEditorActionListener(new OnEditorActionListener() {
				...
				public boolean onEditorAction(TextView v, int actionId, KeyEvent event) {
					...
					if(actionId == EditorInfo.IME_ACTION_DONE || event.getKeyCode() == KeyEvent.KEYCODE_ENTER){
						...
						在这判断输入密码是否正确
						*/
						if(AccountManager.getInstance().checkCurrentPassword(unlock_saleactivity_edittext.getText().toString()))
						{		
							unlock_saleactivity_edittext.setVisibility(View.GONE);
							unlockButton.setVisibility(View.VISIBLE); 
							InputMethodManager inputMgr = (InputMethodManager)mContex.getSystemService(Context.INPUT_METHOD_SERVICE);
