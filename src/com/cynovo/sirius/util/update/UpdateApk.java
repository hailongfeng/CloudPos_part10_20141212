...
import com.cynovo.sirius.fragment.SettingUpdateFragment;
...
import com.cynovo.sirius.util.GetVersionCode;
...
public class UpdateApk {
		...
		new Thread() {
			public void run() {
				...
				if(storeId == null || storeId.equals(""))
					storeId = "0";
				UpdateGotInfo gotInfo = NetworkTool.getApkPath("../apkUpdate/apk_update.php", 
						storeId, apkName, version);
				
				MyLog.d("StartUpdateApk", "StoreId:" + storeId +
						",appName:" + apkName + ",VersionCode:" + version);
				if(gotInfo != null) {
					gotInfo.getVersion();
	...
	private void notNewVersionShow() {
		...
		Dialog dialog = new AlertDialog.Builder(UpdateApk.mactivity)
				...
				.setPositiveButton(Common.getString(R.string.sure),
...
//								downFile(path);
								if (Environment.getExternalStorageState().equals(
									...
									pBar.setButton(CloudPosApp.getInstance().getString(R.string.cancel), new DialogInterface.OnClickListener() {
										@Override
				...
				.setNegativeButton(Common.getString(R.string.notnow_update),
						new DialogInterface.OnClickListener() {
	...
	private void DownloadOK() {
		handler.post(new Runnable() {
			public void run() {
				pBar.cancel();
				MySharedPreferencesEdit.getInstancePublic(CloudPosApp.getInstance()).
					setLastUpdateVersion(GetVersionCode.getAppVersionCode(CloudPosApp.getInstance()));
				MySharedPreferencesEdit.getInstancePublic(CloudPosApp.getInstance()).
					setUpdateApkTime(System.currentTimeMillis());
				InstallApk();
			}
		});
	}
