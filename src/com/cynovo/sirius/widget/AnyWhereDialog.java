...
public class AnyWhereDialog extends Dialog implements OnEditorActionListener {
	...
	public void setProductManagerAtrrrFragment(
			ProductManagerAttrFragment mProductManagerAtrrrFragment) {
		this.mProductManagerAtrrrFragment = mProductManagerAtrrrFragment;
	}

	//
	public AnyWhereDialog(Context context, int width, int height, int x, int y,
	...
	private void saveAtt() {
		if (!mAttr.getText().toString().isEmpty() && !mPrice.getText().toString().isEmpty()) {
			ProductSubAttribute mProductSubAttribute = new ProductSubAttribute(
					-1, -1, mAttr.getText().toString(), mPrice.getText().toString(), 1);
