...
public class ProductAttributeSelector extends ButtonGroupBox implements  OnClickListener{
	...
	public ProductAttributeSelector(Context context,ProductAttribute mAttribute,List<ProductSubAttribute> mSubAttribute,List<ProductSubAttribute> mSelectSubAttribute) {
		...
		for(int i = 0;i < count ; i++)
		{
			...
			if(mSelectSubAttribute == null && i == mDefaultIndex && JavaUtil.isClass(mButtonList.get(i).getClass(), "android.widget.CompoundButton"))
			{
				CompoundButton mb = (CompoundButton) mButtonList.get(i);
				defaultSubID = mSubAttribute.get(i).getProductSubAttributeId();//TODO: add new later
				mb.setChecked(true);
			}
		}
	}
	...
	public void InitButtonState(List<ProductSubAttribute> mSAList)
	{
		...
		for(int i = 0; i < mSAList.size() ;i++)
		{
			if(mSAList.get(i).getProductAttributeID() == mAttribute.getProductAttributeID())
			{
				...
				{
					CompoundButton mb = (CompoundButton) mButtonList.get(getSubAttrIndex(mSAList.get(i).getProductSubAttributeId()));
					mb.setChecked(true);
				}
			}
		}
